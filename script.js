...
function setupSteps(initialData) {
    ...
    steps = [
        ...
        async () => {
            ...
            networkPacket.style.left = '98%'; // End position (matches wire end)
...
nextBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        isSimulating = false;
        resetBtn.disabled = false; // Enable full reset on error
        nextBtn.disabled = true; // Disable Next on error
    } finally {
        // Re-enable buttons if simulation is not finished and no error occurred
        isStepRunning = false;
        if (isSimulating && currentStep < steps.length) {
             nextBtn.disabled = false;
             resetBtn.disabled = false; // Keep full reset enabled
        }
    }
});
